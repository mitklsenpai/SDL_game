...
Game::Game()
{
    ...
    p_pause_button.x = 1248, p_pause_button.y = 0;
}
...
switch(event.type)
    {
        case SDL_MOUSEBUTTONDOWN:
        {
            if(event.button.button == SDL_BUTTON_LEFT)
            {
                ...
                else if(!menu && !paused)
                {
                    ...
                    else if(CheckButton(p_pause_button))
                    {
                        setting_pressed[0] = true;
                        std::cout << "nhan paused" << std::endl;
                    }
                }
                else if(paused)
                {
                    if(CheckButton(p_resume_button))
                    {
                        setting_pressed[1] = true;
                    }
                    else if(CheckButton(p_quit_button_setting))
                    {
        ...
        case SDL_MOUSEMOTION:
        {
            ...
            else if(!menu && !paused)
            {
                ...
                if(CheckButton(p_pause_button))
                {
                    paused_frame++;
                    if(paused_frame == 2)
                        paused_frame = 1;
                    std::cout << "cham paused" << std::endl;
                }
                else
                {
                    paused_frame = 0;
            ...
            else if(paused)
            {
                ...
                if(CheckButton(p_quit_button_setting))
                {
                    quit_frame++;
                    if(quit_frame == 2)
                        quit_frame = 1;
...
{
    Setclip_and_Render(des, p_play, play_button, play_button_frame, "images//Play_button.png", WIDTH_BUTTON, HEIGH_BUTTON);
    Logo = IMG_LoadTexture(des, "images//Logo.png");
    SDL_RenderCopy(des,Logo,NULL,&r_logo);
}

void Game::Setclip_and_Render(SDL_Renderer *des, SDL_Point &point, SDL_Texture *&texture, int &frame, const char* name, int wid, int hei)
{
...
void Game::Replay(SDL_Renderer*des, TTF_Font* font, bool &game_event, bool &is_quit, MainObject &player, std::vector<SmallEnemy*> &Spawner, std::vector<Exp*> &exp_list, std::vector<Nuke*> &nuke_list)
{
    ...
    Setclip_and_Render(des, p_replay, Replay_button, replay_button_frame, "images//Replay_button.png", WIDTH_BUTTON, HEIGH_BUTTON);
    Setclip_and_Render(des, p_quit, Quit_button, quit_button_frame, "images//Quit_button.png", WIDTH_BUTTON, HEIGH_BUTTON);
    game_event = false;
    Spawner.clear();
    exp_list.clear();
    nuke_list.clear();
...
{
    Setclip_and_Render(des, p_pause_button, Pause_button, paused_frame, "images//Paused_button.png", 32, 32);
    if(setting_pressed[0] == true)
    {
        FreeButton(Pause_button);
        Setclip_and_Render(des, p_resume_button, Resume_button, resume_frame, "images//Resume_button.png", 32, 32);
        Setclip_and_Render(des, p_quit_button_setting, Quit_button_setting, quit_frame, "images//quit_button_setting.png", 32, 32);
